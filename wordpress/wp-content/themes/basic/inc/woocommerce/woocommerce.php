...
//				add_action('woocommerce_before_shop_loop_item_title',array($this,'woocommerce_template_loop_product_thumbnail'),10);
		...
		public function woocommerce_template_loop_product_thumbnail(){
			...
			$output='';
			$class = 'image-no-effect img-responsive';
			if(has_post_thumbnail()){
				$attachment_ids = $product->get_gallery_image_ids();
				if($attachment_ids) {
					$class = 'image-hover img-responsive';
					$output.=wp_get_attachment_image($attachment_ids[0],'shop_catalog',false,array('class'=>"attachment-shop_catalog image-effect img-responsive"));
				}
				$output.=get_the_post_thumbnail( $post->ID,'shop_catalog',array('class'=>$class) );
			}else{
				$output .= '<img src="'.wc_placeholder_img_src().'" alt="'.__('Placeholder' , 'basic').'" class="'.$class.'" width="'.$placeholder_width.'" height="'.$placeholder_height.'" />';
			}
			echo $output;
		}
