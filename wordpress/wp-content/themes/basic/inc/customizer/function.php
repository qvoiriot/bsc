...
function wpo_cst_headerlayouts(){
    ...
    $files = glob($path  );
    $headers = array( '' => __('Default', 'basic' ) );
    if(count($files)>0){
        foreach ($files as $key => $file) {
...
function wpo_cst_css_profiles(){
    $path = WPO_FRAMEWORK_CUSTOMZIME_STYLE.'/*.css';
    $files = glob($path);
    $skins = array( 'nouse' => __('No Use', 'basic') );
...
function get_footerbuilder_profiles() {
