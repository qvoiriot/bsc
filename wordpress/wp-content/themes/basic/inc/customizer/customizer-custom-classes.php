...
!important: All theme profiles are stored in folder YOURTHEME/css/customze, it need permission 0755 to put files inside', 'basic').'
            </div>' .'<div><a href="'.admin_url('themes.php?page=wpo_livethemeedit').'">'.__('Create Profile Now', 'basic').'</a></div>';
            printf( 
    ...
    class WPO_Layout_DropDown extends WP_Customize_Control{
        ...
        public function render_content(){
            ...
            $layouts = array(
                '0-1-0' => __('Fullwidth', 'basic'),
                '1-1-0' => __('Left - Main Sidebar', 'basic'),
                '0-1-1' => __('Main - Right Sidebar', 'basic'),
                '1-1-1' => __('Left - Main - Right Sidebar', 'basic'),
            ...
            foreach( $layouts as $key => $layout ){ 
                $v = $this->value() ? $this->value() : '0-1-0' ;  // echo $this->value();die; 
        ...
        public function render_content() {
            $dropdown = wp_dropdown_categories( 
                array( 
                    ...
                    'echo'             => 0,
                    'hide_empty'       => false,
                    'show_option_none' => '&mdash; ' . __('Select', 'basic') . ' &mdash;',
                    'hide_if_empty'    => false,
                    'selected'         => $this->value(),
                )
            );
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class WPO_Customize_Control_Textarea extends WP_Customize_Control {
    public $type = 'textarea';
    public function render_content() {
      ?>
        <label>
          <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
          <textarea <?php $this->link(); ?> rows="5" style="width: 98%;"><?php echo esc_textarea( $this->value() ); ?></textarea>
        </label>
      <?php
    }
  }
