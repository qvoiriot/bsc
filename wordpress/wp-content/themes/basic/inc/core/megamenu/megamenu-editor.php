...
if(!class_exists('WPO_MegamenuEditor')){
		...
		class WPO_MegamenuEditor{
			...
			public function adminLoadMenu(){ 
				...
				add_theme_page( 'Megamenu', __("Megamenu Editor", 'basic'), 'administrator', 'wpo_megamenu', array($this,'megaMenuPage') );
				add_theme_page( 'Megamenu', __("Megamenu Vertical Editor", 'basic'), 'administrator', 'wpo_megamenu_vertical', array($this,'megaMenuVerticalPage') );
			}
			...
			public function ajax_shortcode_save(){
				...
				if($id==0){
					...
					$response['title']=$name;
					$response['message'] = __('Widgets published', 'basic');
					$response['type_widget'] = $type;
				}else{
					$response['type']='update';
					$response['message'] = __('Widgets updated', 'basic');
					$response['title']=$name;
					$response['id']=$id;
