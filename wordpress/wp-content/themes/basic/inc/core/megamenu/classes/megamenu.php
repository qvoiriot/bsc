	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( ! empty( $item->text_customize ) ){
			switch ( $item->text_customize ) {
			 	case 'text_new':
			 		$text_customize  = __('New', 'basic');
		 			break;

		 		case 'text_hot':
		 			$text_customize  = __('Hot', 'basic');
	 				break;

	 			case 'text_featured':
			 		$text_customize  = __('Featured', 'basic');
		 			break;
			...
			$text_customize  = ! empty( $item->text_customize ) ? '<span class="' . $item->text_customize . ' text-label">'.$text_customize.'</span>' : '';
		}
				...
				$output .= '<div class="row">';
					foreach( $row->cols as $col ){
						...
						$colwidth = isset($col->colwidth) ? $col->colwidth:'';
						 $output .= '<div class="mega-col col-md-'.$colwidth.' '.$colclass.'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
