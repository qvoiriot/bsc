...
if(!function_exists('wpo_related_post')){
    function wpo_related_post( $relate_count = -1, $posttype = 'post', $taxonomy = 'category'){
...
if(!function_exists('wpo_getAgo')){
    function wpo_getAgo($timestamp){
        ...
        if ($difference < 60) {
            return $difference.__(" seconds ago",'basic');
        } else {
            $difference = round($difference / 60);
        }

        if ($difference < 60) {
            return $difference.__(" minutes ago",'basic');
        } else {
            $difference = round($difference / 60);
        }

        if ($difference < 24) {
            return $difference.__(" hours ago",'basic');
        }
        else {
        ...
        if ($difference < 7) {
            return $difference.__(" days ago",'basic');
        } else {
            $difference = round($difference / 7);
            return $difference.__(" weeks ago",'basic');
        }
    }
}
...
if(!function_exists('wpo_embed')){
    function wpo_embed( $config ) {
        ...
        if(isset($postconfig[$config['format']])){
            if(preg_match("/vimeo.com\/([^&]+)/i", $postconfig[$config['format']], $aMatch)){
                $content = '<iframe src="http://player.vimeo.com/video/' . $aMatch[1] . '?title=0&byline=0&portrait=0" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
            }else{
...
if(!function_exists('wpo_gallery')){
    function wpo_gallery($size='fullwidth'){
        $galleries = get_post_gallery( get_the_ID(), false );
...
if(!function_exists('wpo_pagination_nav')){
    function wpo_pagination_nav($per_page,$total,$max_num_pages=''){
        ...
        <section class="wpo-pagination">
            <?php global  $wp_query; ?>
            <?php wpo_pagination($prev = __('Previous','basic'), $next = __('Next','basic'), $pages=$max_num_pages ,array('class'=>'pull-left')); ?>
