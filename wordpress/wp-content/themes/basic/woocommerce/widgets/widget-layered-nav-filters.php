	...
	public function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
			...
			if (!is_null($_chosen_attributes)){
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					foreach ( $data['terms'] as $term_id ) {
						...
						echo '<li class="chosen"><a title="' . __( 'Remove filter', 'basic' ) . '" href="' . ( $link ) . '">' . $term->name . '</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'basic' ) . '" href="' . ( $link ) . '">' . __( 'Min', 'basic' ) . ' ' . wc_price( $min_price ) . '</a></li>';
			}

			if ( $max_price ) {
				$link = esc_url( remove_query_arg( 'max_price' ) );
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'basic' ) . '" href="' . ( $link ) . '">' . __( 'Max', 'basic' ) . ' ' . wc_price( $max_price ) . '</a></li>';
			}
