...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		jQuery('#twitter').sharrre({
			...
			enableHover: false,
			enableTracking: true,
			buttons: { twitter: {via: '<?php echo esc_js( of_get_option('twitter-username') ); ?>'}},
		...
		jQuery('#googleplus').sharrre({
			...
			enableHover: false,
			enableTracking: true,
			urlCurl: '<?php echo esc_js( get_template_directory_uri() .'/js/sharrre.php' ); ?>',
			click: function(api, options){
				api.simulateClick();
		...
		jQuery('#pinterest').sharrre({
			...
			buttons: {
			pinterest: {
				description: '<?php echo esc_js( the_title() ); ?>'<?php if( has_post_thumbnail() ){ ?>,media: '<?php echo esc_js( wp_get_attachment_url( get_post_thumbnail_id($post->ID) ) ); ?>'<?php } ?>
				}
			},
			click: function(api, options){
				api.simulateClick();
