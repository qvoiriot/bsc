...
<?php if ( has_post_format( 'audio' ) ): // Audio ?>
	...
	jQuery(document).ready(function(){
		if(jQuery().jPlayer) {
			jQuery("#jquery-jplayer-<?php esc_js( the_ID() ); ?>").jPlayer({
				ready: function () {
					jQuery(this).jPlayer("setMedia", {
						<?php if(in_array('mp3',$formats)) { echo 'mp3: "'.esc_js( $meta['_audio_mp3_url'][0] ).'",'."\n"; } ?>
						<?php if(in_array('m4a',$formats)) { echo 'm4a: "'.esc_js( $meta['_audio_mp3_url'][0] ).'",'."\n"; } ?>
						<?php if(in_array('oga',$formats)) { echo 'oga: "'.esc_js( $meta['_audio_ogg_url'][0] ).'",'."\n"; } ?>
					});
				},
			 ...
			 var firstImage = jQuery('#flexslider-<?php echo esc_js( the_ID() ); ?>').find('img').filter(':first'),
				checkforloaded = setInterval(function() {
					...
					if (image.complete || image.readyState == 'complete' || image.readyState == 4) {
						...
						jQuery('#flexslider-<?php echo esc_js( the_ID() ); ?>').flexslider({
							animation: "fade",
							slideshow: true,
							directionNav: true,
							controlNav: true,
				...
				<ul class="slides">
					<?php foreach ( $images as $image ): ?>
						<li>
							...
							<img src="<?php echo esc_url( $imageid[0] ); ?>" alt="<?php echo esc_attr($image->post_title); ?>">
